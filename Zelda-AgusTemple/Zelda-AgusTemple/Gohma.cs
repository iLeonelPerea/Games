...
namespace Zelda_AgusTemple
{
    ...
    /// </summary>
    public class Gohma : Microsoft.Xna.Framework.DrawableGameComponent
    {
        Texture2D textura1, textura2, actual;
        Vector2 pos, posL;
        int contador, contador1, contadorh, vida;
        public bool bHijo;
        public Gohma(Game game, int x, int y)
            : base(game)
        {
            textura1 = Game.Content.Load<Texture2D>("Enemigos/Gohma/Gohma1");
            textura2 = Game.Content.Load<Texture2D>("Enemigos/Gohma/Gohma2");
            actual = textura1;
            contador = 0;
            contador1 = 0;
            contadorh = 0;
        ...
        {
            return new Rectangle((int)(pos.X), (int)(pos.Y), 96, 32);
        }
        ...
        public void Mover()
        {
            ...
            while (i < 10)
            {
                if (r.NextDouble() < limitex)
                {
                    ...
                    {
                        pos.X += (float)r.NextDouble() * 2;
                        pos.Y += (float)r.NextDouble() * 2;
                    }
                    else
                    {
                        pos.X += (float)r.NextDouble() * 2;
